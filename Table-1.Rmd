...
coxdf <- 
  ...
  mutate(ECOG = ecog,
         ...
         `CCI` = ifelse(comorb.score < 5, "0-4", "5+"))
...
```{r table 1}
listVars <- c("BMI", "Age", "Sex", "ECOG", "CCI", "Cancer", "Staging", "Immunotherapy", "ATB within 28 days of IO", "CS within 28 days of IO")
catVars <- c("Sex", "Cancer", "Immunotherapy", "ECOG", "Staging", "ATB within 28 days of IO", "CS within 28 days of IO", "CCI")
...
catVars <- c("Sex", "Cancer", "Immunotherapy", "ECOG", "Staging", "ATB within 28 days of IO", "CS within 28 days of IO", "CCI")
table1 <- CreateTableOne(vars = listVars,
                         strata = "ATB within 28 days of IO",
                         data = coxdf,
                         factorVars = catVars,
...
table1 <- CreateTableOne(vars = listVars,
                         data = coxdf,
                         factorVars = catVars,
                         strata = "CS within 28 days of IO",
                         argsNonNormal = listVars,
                         argsNormal = list(NULL)
)
table1
