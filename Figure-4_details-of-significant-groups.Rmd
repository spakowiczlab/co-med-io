...
ggplot(data = allpvaltime, aes(x = leftanchor, y = hazard.ratio, color = source, fill = source)) +
  geom_line() +
  geom_ribbon(aes(ymin = low.bound, ymax = upper.bound), alpha = 0.5) +
...
heatplot <- heatclass %>%
  ggplot(aes(fct_relevel(AbxClass, bacorder), fct_relevel(cancer, "Other"))) +
  ...
  scale_y_discrete(breaks = c("Other", "Bladder Cancer", "Head and Neck Carcinoma", "Melanoma", "Non-Small Cell Lung Cancer", "Renal Cell Carcinoma", "Sarcoma"), labels = c("Other", "Bladder Cancer", "Head and Neck\nCarcinoma", "Melanoma", "Non-Small Cell\nLung Cancer", "Renal Cell\nCarcinoma", "Sarcoma")) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12), axis.text = element_text(size = 12), plot.margin = margin(0,0,0,0))
...
dbidheatplot <- heatster %>%
  ggplot(aes(fct_relevel(Steroid.ID, as.character(dbidtree$labels$label)), fct_relevel(cancer, "Other"))) +
  geom_tile(aes(fill = p.value)) +
  geom_point(aes(shape = labcol), size = 5) +
  scale_shape_manual(breaks = c("black", "red"), values = c("-", "+"), labels = c("HR>1", "HR<1"), name = "Direction") +
  labs(x = "", y = "") +
  ...
  scale_x_discrete(breaks = c("Methylprednisone", "Prednisone", "Dexamethosone", "Hydrocortisone", "Other.Steroid", "Triamcinolone"), labels = c("Methylprednisone", "Prednisone", "Dexamethosone", "Hydrocortisone", "Other Steroid", "Triamcinolone")) +
  scale_y_discrete(breaks = c("Other", "Bladder Cancer", "Head and Neck Carcinoma", "Melanoma", "Non-Small Cell Lung Cancer", "Renal Cell Carcinoma", "Sarcoma"), labels = c("Other", "Bladder Cancer", "Head and Neck\nCarcinoma", "Melanoma", "Non-Small Cell\nLung Cancer", "Renal Cell\nCarcinoma", "Sarcoma")) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12), axis.text.y = element_text(size = 12), plot.margin = margin(0,0,0,0)) 
