...
resp.key <- as.data.frame(cbind(search_string = litmics,
                                resp.stat, resp.ref,
                                fixed = "hold"))
...
resp.key <- taxa %>%
  ...
  mutate(tip.label = gsub("_ott\\d+", "", tip.label),
         tip.label = gsub("_", " ", tip.label))
...
tree.plot <- ggtree(tree)  %<+%
  ...
  geom_cladelabel(node = 30, label = "Bacteroidetes \n(Gopalakrishnan)", geom = "text", color = "darkred",
                  offset = -2.3, offset.text = -2.7, align = TRUE) +
  geom_cladelabel(node = 22, label = "Firmicutes \n(Gopalakrishnan)", geom = "text", color = "royalblue",
                  offset = -4.3, offset.text = -2.6, align = TRUE) +
  xlim(c(0,11.5)) 
...
susc <- as.data.frame(cbind(taxon = order, 
                            Cephalosporins = ceph, 
                            Piperacillin = pip, 
                            Amoxicillin = amclac,  
                            Clindamycin = clin, 
...
```{r construct heatmap}
heat <- ggplot(data = susc, aes(fct_relevel(taxon, rev(order)), 
