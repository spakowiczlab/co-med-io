...
```{r}
lasso95$whichco <- gsub("Stage IV", "`Stage IV`", lasso95$whichco)
lasso95$whichco <- gsub("Male", "Sex", lasso95$whichco)
```
...
for(i in 1:length(allcancers)){
  c <- as.character(lasso95[i,1])
  tmp <- db %>%
    filter(cancer.name == c)

  f <- as.formula(paste("Surv(days, vitalstatus) ~", lasso95[i,4]))
  if(lasso95[i,3] != 0){
    mods[[c]] <- coxph(f, data = tmp2) %>%
      tidy() %>%
...
```{r}
names(mypal) <- c("Bladder Cancer", "Head and Neck Carcinoma", "Melanoma", "Non-Small Cell Lung Cancer", "Other", "Renal Cell Carcinoma", "Sarcoma")
mods <- mods %>%
  filter(conf.high < Inf)
mods$term <- gsub("SexMale", "Sex", mods$term)

ggplot(data=mods, aes(x = cancer,y = estimate, ymin = conf.low, ymax = conf.high ))+
  geom_pointrange(aes(color = cancer)) +
  geom_hline(aes(fill = cancer), yintercept = 1, linetype =2 )+
  labs(x = "", y= "") +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high, color = cancer)) + 
  scale_color_manual(breaks = c("Bladder Cancer", "Head and Neck Carcinoma", "Melanoma", "Non-Small Cell Lung Cancer", "Other", "Renal Cell Carcinoma", "Sarcoma"), values = mypal, name = "Cancer") +
  facet_wrap(~term, strip.position = "left", nrow = 7, scales = "free_y") +
  theme_bw() +
  ...
  coord_flip() 
  #ggsave("../figures/Figure-4/forest_split-cancers.png")
